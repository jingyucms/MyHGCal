...
class HGCalScintRecHitStudy : public edm::one::EDAnalyzer<edm::one::WatchRuns, edm::one::SharedResources> {
public:
  explicit HGCalScintRecHitStudy(const edm::ParameterSet&);
  ~HGCalScintRecHitStudy() override {}
...
private:
  ...
  const int verbosity_, nbinR_, nbinZ_, nbinEta_, nbinPhi_;
...
HGCalScintRecHitStudy::HGCalScintRecHitStudy(const edm::ParameterSet& iConfig)
    : nameDetector_(iConfig.getParameter<std::string>("detectorName")),
      ...
      nbinEta_(iConfig.getUntrackedParameter<int>("nBinEta", 74)),
      nbinPhi_(iConfig.getUntrackedParameter<int>("nBinPhi", 288)),
      ...
      etamin_(iConfig.getUntrackedParameter<double>("etaMin", 1.42)),
  ...
  edm::LogVerbatim("HGCalStudies") << "Initialize HGCalRecHitStudy for " << nameDetector_ << " with i/p tag " << temp
                                      << " Flag " << ifHCAL_ << ":" << ifNose_ << ":" << verbosity_;
}

void HGCalScintRecHitStudy::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<std::string>("detectorName", "HGCalHEScintillatorSensitive");
  desc.add<edm::InputTag>("source", edm::InputTag("HGCalRecHit", "HGCHEBRecHits"));
  ...
  desc.addUntracked<int>("nBinEta", 74);
  desc.addUntracked<int>("nBinPhi", 288);
  ...
  desc.addUntracked<double>("etaMin", 1.42);
  desc.addUntracked<double>("etaMax", 3.0);
  descriptions.add("hgcalRecHitStudyHEScintillator", desc);
}

void HGCalScintRecHitStudy::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (nameDetector_ == "HCal") {
    ...
    if (!geom.isValid())
      edm::LogWarning("HGCalStudies") << "Cannot get valid HGCalGeometry Object for " << nameDetector_;
    ...
    if (ifHCAL_) {
      ...
      if (hbhecoll.isValid()) {
        if (verbosity_ > 0)
          edm::LogVerbatim("HGCalStudies") << nameDetector_ << " with " << hbhecoll->size() << " element(s)";
        ...
        ok = false;
        edm::LogWarning("HGCalStudies") << "HBHERecHitCollection Handle does not exist !!!";
      }
    } else {
      ...
      if (hbhecoll.isValid()) {
        ...
        ok = false;
        edm::LogWarning("HGCalStudies") << "HGChebRecHitCollection Handle does not exist !!!";
      }
    }
  } else {
    ...
    if (theRecHitContainers.isValid()) {
      ...
      ok = false;
      edm::LogWarning("HGCalStudies") << "HGCRecHitCollection Handle does not exist !!!";
    }
  }
  if (ok)
    fillHitsInfo();
  edm::LogVerbatim("HGCalStudies") << "Event " << iEvent.id().event() << " with " << ntot << " total and " << nused
                                      << " used recHits";
}

template <class T1, class T2>
void HGCalScintRecHitStudy::recHitValidation(DetId& detId, int layer, const T1* geom, T2 it) {
  ...
  if (verbosity_ > 1)
    edm::LogVerbatim("HGCalStudies") << " --------------------------   gx = " << globalx << " gy = " << globaly
...
void HGCalScintRecHitStudy::fillOccupancyMap(std::map<int, int>& OccupancyMap, int layer) {
...
void HGCalScintRecHitStudy::fillHitsInfo() {
...
void HGCalScintRecHitStudy::fillHitsInfo(HitsInfo& hits) {
...
void HGCalScintRecHitStudy::beginRun(edm::Run const&, edm::EventSetup const& iSetup) {
  if (nameDetector_ == "HCal") {
  ...
  edm::LogVerbatim("HGCalStudies") << "Finds " << layers_ << " layers for " << nameDetector_;
  ...
  for (unsigned int il = 0; il < layers_; il++) {
    ...
    EtaPhi_Plus_.push_back(fs->make<TH2D>(histoname, "Occupancy", nbinEta_, etamin_, etamax_, nbinPhi_, -M_PI, M_PI));
    sprintf(histoname, "EtaPhi_Minus_Layer_%d", ilayer);
    EtaPhi_Minus_.push_back(fs->make<TH2D>(histoname, "Occupancy", nbinEta_, -etamax_, -etamin_, nbinPhi_, -M_PI, M_PI));
  ...
  h_EtaPhi_ = fs->make<TH2D>(histoname, title, nbinEta_, etamin_, etamax_, nbinPhi_, -M_PI, M_PI);
}
...
DEFINE_FWK_MODULE(HGCalScintRecHitStudy);
